...
public class NewGenerationStrategy {
    ...
    public LifeForm generateNextGen(LifeForm oldLifeForm){
        if(oldLifeForm == null) return null;
        ...
        for(NextGenStrategy rule : rules){
            if(rule.strategyApplies(world,oldLifeForm.getLocation().x,oldLifeForm.getLocation().y)){
                newLifeForm = new LifeForm(rule.getNextGen(),oldLifeForm.getAliveRepresentation(),oldLifeForm.getDeadRepresentation());
            }
        }
        return newLifeForm;
    }
